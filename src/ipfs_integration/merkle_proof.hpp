...
/*
  ...
  Required Methods (from specification):
    ...
    std::vector<uint8_t> GenerateProof(const std::string &filePath, const std::vector<size_t> &offsets)
...
public:
    ...
    */
    std::vector<uint8_t> GenerateProof(const std::string &filePath, const std::vector<size_t> &offsets)
    {
        ...
        {
            if (off < chunks.size()) { validOffsets++; }
        }
        ...
        Logger::getInstance().info("[MerkleProof] Proof generated successfully. Root: " + merkleRoot);
        return proofData;
    }
    ...
    /*
      VerifyProof
      --------------------------------
      1) Parses the proofData for chunk size, total chunks, offsets, chunk data, sibling paths, and root.
      2) For each offset block:
    ...
    */
    bool VerifyProof(const std::vector<uint8_t> &proofData)
    {
        ...
        auto readU32 = [&](uint32_t &val) -> bool {
            if (readPos + 4 > proofData.size()) return false;
        ...
        if (!readU32(chunkSize)) { return false; }

        // 2) totalChunks
        uint32_t totalChunks = 0;
        if (!readU32(totalChunks)) { return false; }

        // 3) numberOfOffsets
        uint32_t numOffsets = 0;
        if (!readU32(numOffsets)) { return false; }
        ...
        for (uint32_t i = 0; i < numOffsets; i++)
        {
            ...
            // offsetIndex
            if (!readU32(op.offsetIndex)) { return false; }

            // chunkDataLength + chunkData
            uint32_t cdl = 0;
            if (!readU32(cdl)) { return false; }
            if (readPos + cdl > proofData.size()) { return false; }
            op.chunkData.insert(op.chunkData.end(), 
                                proofData.begin() + readPos,
                                proofData.begin() + readPos + cdl);
            readPos += cdl;

            // pathLength
            uint32_t pathLen = 0;
            if (!readU32(pathLen)) { return false; }
            for (uint32_t p = 0; p < pathLen; p++)
            {
                uint32_t hashSize = 0;
                if (!readU32(hashSize)) { return false; }
                if (readPos + hashSize > proofData.size()) { return false; }
        ...
        if (!readU32(rootHashSize)) { return false; }
        if (readPos + rootHashSize > proofData.size()) { return false; }
        ...
        for (auto &op : offsetProofs)
        {
            ...
            {
                Logger::getInstance().warn("[MerkleProof] Mismatch at offset " 
                        + std::to_string(op.offsetIndex) 
                        + ", computed root: " + currentHash 
                        + " vs. stored root: " + merkleRoot);
                return false;
            }
        }

        // If we pass all checks, it's good
        Logger::getInstance().info("[MerkleProof] Proof verified successfully. Root: " + merkleRoot);
        return true;
    }
...
private:
    ...
    */
                   ...
                   std::vector<std::vector<uint8_t>> &chunks)
    {
        ...
        while (offset < totalSize)
        {
            ...
            std::vector<uint8_t> chunk(fileData.begin() + offset, fileData.begin() + offset + thisSize);
            ...
            for (size_t i = 0; i < currentLevel.size(); i += 2)
            {
                if (i + 1 < currentLevel.size())
                {
                    ...
                    combined.insert(combined.end(), 
                                    currentLevel[i].begin(), currentLevel[i].end());
                    combined.insert(combined.end(), 
                                    currentLevel[i + 1].begin(), currentLevel[i + 1].end());
    ...
    */
    std::vector<std::string> getMerklePath(const std::vector<std::vector<std::string>> &treeLevels, size_t leafIndex)
    {
